...
public class RncLightyModuleSmokeTest {
    ...
    public void rncLightyModuleDefaultConfigTest() throws Exception {
        ...
        final var http1Client = HttpClient.newHttpClient();
        final var httpResponse = http1Client.send(createGetRequest(HTTP_URI + TOPOLOGY_PATH), BodyHandlers.ofString());
        assertEquals(HttpStatus.OK_200, httpResponse.statusCode());
        assertTrue(rncModule.close());
    }

    @Test
    public void rncLightyModuleCustomConfigTest() throws Exception {
        final var resource = RncLightyModuleSmokeTest.class.getResource("/smokeTestConfig.json");
        ...
        assertTrue(rncModule.initModules());
        final var http2Client = getHttp2Client();
        final var httpResponse = http2Client.send(createGetRequest(HTTPS_URI + TOPOLOGY_PATH), BodyHandlers.ofString());
        assertEquals(HttpStatus.OK_200, httpResponse.statusCode());
        assertTrue(rncModule.close());
    }
    ...
    private static HttpClient getHttp2Client() throws Exception {
